...
func TestRoutes_ZonesEndpoint(t *testing.T) {
	cfg := createTestConfig()
	server := api.New(cfg, nil)

	w := performRequest(server.Engine(), "GET", "/api/v1/zones", "")
...
func TestRoutes_PostZones_NotImplemented(t *testing.T) {
	cfg := createTestConfig()
	server := api.New(cfg, nil)

	w := performRequest(server.Engine(), "POST", "/api/v1/zones", `{"name":"test.com"}`)
