...
// Design Philosophy:
//
// Rate limiting happens BEFORE DNS packet parsing. This is crucial because:
//   - Protects upstream servers from load spikes
//   - Minimal CPU overhead for dropped packets (no parsing)
//   - Prevents resource exhaustion from malformed or amplified queries
//
// Three-Tier Rate Limiting:
//
// Rate limiting is applied at three levels. A request must pass all three to be allowed:
//
//  1. Global: Overall server-wide query rate limit (e.g., 100,000 QPS)
//     - Single global limit for all clients combined
//     - Prevents total server overload
//
//  2. Prefix: Per-network prefix limit (/24 for IPv4, /64 for IPv6)
//     - Groups clients by network (e.g., 192.0.2.0/24)
//     - Prevents single subnet from overwhelming the server
//     - Still allows multiple clients in the same network
//
//  3. IP: Per source IP limit (e.g., 5,000 QPS)
//     - Individual client limit
//     - Prevents single client from exhausting resources
//
// Token Bucket Algorithm:
//
// Each level maintains a token bucket. Tokens are:
//   - Added at a steady rate (QPS)
//   - Consumed when queries arrive (one token per query)
//   - Limited by bucket capacity (Burst)
//
// A request is allowed if there are available tokens at all three levels.
// Bursts up to the configured limit are allowed, then rate-limited back to QPS.
